...
#### PyInstaller --windowed 模式 stdin 问题（已修复）

- **现象**：使用 `--windowed` 模式打包后运行报错：
  ```
  AttributeError: 'NoneType' object has no attribute 'isatty'
  RuntimeError: lost sys.stdin
  ```
- **原因**：`--windowed` 模式下 `sys.stdin/stdout/stderr` 为 `None`，uvicorn 日志配置失败
- **解决方案**：
  1. 在 `launcher.py` 开头添加 stdin/stdout 修复代码：
     ```python
     import io
     ...
     ```
  2. 使用自定义日志配置替代 uvicorn 默认配置
  3. 改用 `--console` 模式（显示控制台窗口）

### 10.5 GitHub 仓库

- **仓库地址**：https://github.com/Baishukai/xiaobai-watermark-remover
- **详细指南**：见 `GITHUB_GUIDE.md`

发布 Release 步骤：
1. 压缩 `dist/小白AI图片去水印/` 目录
